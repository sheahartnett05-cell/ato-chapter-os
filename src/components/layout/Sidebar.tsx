...
const mainNav = [
  ...
  { to: '/announcements', label: 'Announcements', icon: Megaphone, perm: 'canPostAnnouncements' as const },
  { to: '/members', label: 'Members', icon: Users, perm: 'canManageRoster' as const },
  { to: '/calendar', label: 'Calendar', icon: CalendarDays },
  { to: '/excuses', label: 'Excuses', icon: ClipboardCheck, perm: 'canAccessExecTools' as const },
  { to: '/library-hours', label: 'Library Hours', icon: BookOpen },
  { to: '/tables/t1', label: 'Tables', icon: Table2 },
]
...
const adminNav = [
  { to: '/judicial-board', label: 'J-Board', icon: Scale, perm: 'canViewJBoardCases' as const },
  { to: '/committees', label: 'Committees', icon: UsersRound, perm: 'canAccessExecTools' as const },
  { to: '/chapter-setup', label: 'Chapter Setup', icon: Settings, perm: 'canAccessAdminSettings' as const },
  { to: '/exec-slides', label: 'Exec Slides', icon: Presentation, perm: 'canAccessExecTools' as const },
]
...
export function Sidebar() {
  ...
  const visibleMain = mainNav.filter((item) => !item.perm || permissions[item.perm])
  const visibleAdmin = adminNav.filter((item) => !item.perm || permissions[item.perm])
  const showRecruitment = permissions.canManageRecruitment
  const displayName = `${profile.firstName} ${profile.lastName}`.trim() || 'Member'

  return (
    <aside className="theme-sidebar fixed inset-y-0 left-0 z-30 flex w-56 flex-col">
      ...
      <div className="border-t border-white/10 p-3">
        ...
        >
          <div
            className="flex h-8 w-8 shrink-0 items-center justify-center font-mono text-[10px] font-semibold"
            style={{
              backgroundColor: 'var(--accent)',
              color: 'var(--accent-foreground)',
            }}
          >
            {profile.avatar || chapter.letters.slice(0, 2)}
          </div>
